...
from .models import Inquiry, InquiryEquipment, Quotation, QuotationEquipment, PaymentImage
...
@user_passes_test(lambda u: u.is_anonymous or (u.useraccount.user_type != "MM" and u.useraccount.user_type != "FI" and
                                               u.useraccount.user_type != "EM" and u.is_superuser is False),
                  login_url='errorpage')
...
def transactions(request):
    ...
    transactions_html = loggedinuser.is_superuser or loggedinuser.useraccount.user_type == "EM" or loggedinuser.useraccount.user_type == "CU"

    if transactions_html:
        if loggedinuser.is_superuser or loggedinuser.useraccount.user_type == "EM":
            ...
            context = {'transactions': combined_transactions, 'transaction_type': "AL"}
        else:
...
def transactionsfiltered(request, transaction_type):
    loggedinuser = request.user
    if loggedinuser.is_superuser or loggedinuser.useraccount.user_type == "EM" or loggedinuser.useraccount.user_type == "FI":
...
def view_rental_reports(request):
    customers = UserAccount.objects.filter(user_type="CU")
    ...
    return render(request, "rentalreport.html", {"customers": customers})


@user_passes_test(lambda u: u.useraccount.user_type == "CU", login_url='errorpage')
def addImage(request, quotation_pk):
    quotation = get_object_or_404(Quotation.objects.all(), pk=quotation_pk)
    image = request.FILES.get('paymentImage')
    PaymentImage.objects.create(quotation=quotation, payment_image=image)
