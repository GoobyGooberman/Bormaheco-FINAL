...
class Equipment(models.Model):
    ...
    def tomorrow_engagement(self):
        from rental.models import Inquiry
        inquiries = Inquiry.objects.filter(inquiryequipment__equipment=self).filter(start_date__lte=timezone.now() + timezone.timedelta(days = 1)).\
            filter(end_date__gte=timezone.now() + timezone.timedelta(days=1)).filter(status="CO")
        if not inquiries:
            return False
            # no current confirmed engagement tomorrow
        else:
            return True
            # there is a current confirmed engagement tomorrow
